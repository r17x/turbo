...
func TestBuildPackageGraph_DuplicateNames(t *testing.T) {
	...
	expectedErr := regexp.MustCompile("Failed to add workspace \"same-name\" from apps/[a|b], it already exists at apps/[a|b]")
	testifyAssert.Regexp(t, expectedErr, actualErr)
}
